...
\end{equation}
where the rate of decay then grows exponentially with distance traveled $z$ through the medium. The decay term $e^{-\chi^{(1)}_I k z / 2}$ is known as Beer-Lambert law~\cite{MacRae}.
...
\begin{equation}
    \vec{\mu}_{ji} = \bra{j}\hat{\vec{d}}\ket{i} = e\bra{j}\hat{\vec{r}}\ket{i}
\end{equation}
...
If $\vec{\mu}_{ji} = 0$, then the transition between states $\ket{i}$ and $\ket{j}$ is a `forbidden' dipole transition, otherwise the transition is an allowed dipole transition. This criterion enforces the dipole selection rules that~\cite{Atomic}
...
\begin{align}\label{ConservationEqs}
    &\hbar \omega_a + \hbar \omega_b = \hbar \omega_c + \hbar \omega_d\\
    &\vec{k}_a + \vec{k}_b = \vec{k}_c + \vec{k}_d
\end{align}
...
\subsubsection{Combined Solution Algorithm}
...
\begin{algorithm}[H]\label{Alg}
    \caption{Lindblad-Maxwell-Bloch Algorithm}
    \begin{algorithmic}[1]
